...
function calculatePersona(answers = []) {
  ...
  if (highestScoringPersonas.length === 1) {
    // No tie - clear winner
    matchedPersona = personas[highestScoringPersonas[0]];
  } else {
    // There's a tie - implement tie-breaking logic
    
    // Option 1: Return the first matched persona in tie
    // matchedPersona = personas[highestScoringPersonas[0]];
    
    // Option 2: Randomly select between tied personas for surprise
    const randomIndex = Math.floor(Math.random() * highestScoringPersonas.length);
    matchedPersona = personas[highestScoringPersonas[randomIndex]];
    
    // For debugging
    // console.log(`Tie between: ${highestScoringPersonas.join(', ')}. Selected: ${highestScoringPersonas[randomIndex]}`);
  }
...
module.exports = {
  calculatePersona,
  personas,
  scoringMap // Export the scoring map for potential use elsewhere or testing
};